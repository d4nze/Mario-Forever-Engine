...
{
    ImGuiWindow*        Window;         // Init,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->UIWindow)
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImVec2              LastPos;
    float               Alpha;                  // UIWindow opacity (when dragging dockable windows/viewports we make them transparent)
    float               LastAlpha;
...
struct ImGuiSettingsHandler
{
    ...
    void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuiSettingsHandler* handler, const char* name);              // Read: Called when entering into a new ini entry e.g. "[UIWindow][Name]"
...
struct ImGuiContext
{
    ...
    ImGuiWindow*            CurrentWindow;                      // UIWindow being drawn into
    ImGuiWindow*            HoveredWindow;                      // UIWindow the mouse is hovering. Will typically catch mouse inputs.
...
{
    ImGuiContext*           Ctx;                                // Parent UI context (needs to be set explicitly by parent).
    char*                   Name;                               // UIWindow name, owned by the window.
    ...
    ImVec2                  WindowPadding;                      // UIWindow padding at the time of Begin().
    float                   WindowRounding;                     // UIWindow rounding at the time of Begin(). May be clamped lower to avoid rendering artifacts with title bar, menu bar etc.
    float                   WindowBorderSize;                   // UIWindow border size at the time of Begin().
    ...
    ImRect                  OuterRectClipped;                   // == UIWindow->Rect() just after setup in Begin(). == window->Rect() for root window.
...
enum ImGuiTabItemFlagsPrivate_
{
    ...
    ImGuiTabItemFlags_Unsorted                  = 1 << 22,  // [Docking] Trailing tabs with the _Unsorted flag will be sorted based on the DockOrder of their UIWindow.
};
...
struct ImGuiTabItem
{
    ...
    ImS32               NameOffset;             // When UIWindow==NULL, offset to name within parent ImGuiTabBar::TabsNames
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiWindow*                InnerWindow;                // UIWindow holding the table data (== OuterWindow or a child window)
